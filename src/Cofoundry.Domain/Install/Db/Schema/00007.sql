...
alter table Cofoundry.PageTemplate modify column CustomEntityModelType nvarchar(400) null;
...
create unique index UIX_PageBlockType_FileName on Cofoundry.PageBlockType (FileName);
...
update Cofoundry.CustomEntityVersionPageBlock as b
inner join Cofoundry.PageTemplateRegion r on b.PageTemplateRegionId = r.PageTemplateRegionId
inner join Cofoundry.PageVersion pv on pv.PageTemplateId = r.PageTemplateId
inner join Cofoundry.PageTemplate pt on pt.PageTemplateId = pv.PageTemplateId
inner join Cofoundry.Page p on p.PageId = pv.PageId
set b.PageId = pv.PageId
...
alter table Cofoundry.CustomEntityVersionPageBlock modify column PageId int not null;
...
select * from CustomEntityCTE;
-- update CustomEntityCTE set DisplayVersion = RowNum;


alter table Cofoundry.PageVersion modify column DisplayVersion int not null;
alter table Cofoundry.CustomEntityVersion modify column DisplayVersion int not null;

create unique index UIX_PageVersion_DraftVersion2 
